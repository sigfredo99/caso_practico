$(document).ready(function () {
  tableChannels = $("#tableChannels").DataTable({
    columnDefs: [
      {
        targets: -1,
        data: null,
  ...
  $("#formChannels").submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
      url:
        action == 0 ? "Actions/Channels/Save.php" : "Actions/Channels/Edit.php",
      type: "POST",
      dataType: "json",
      data: data,
      success: function (data) {
        ...
        name = data[0].name;
        if (action == 0) {
          tableChannels.row.add([id, name]).draw();
        } else {
          tableChannels.row(row).data([id, name]).draw();
        }
      },
    });
...
$(document).on("click", ".btnDelete", function () {
  ...
  if (response) {
    $.ajax({
      url: "Actions/Channels/Delete.php",
      type: "POST",
      dataType: "json",
      data: { id: id },
      success: function () {
        tableChannels.row(row.parents("tr")).remove().draw();
      },
    });
  }
});
